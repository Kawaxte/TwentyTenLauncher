...
public final class LauncherUtils {
  ...
  static {
    ...
    try {
      githubReleasesUrl = new URL(
          new StringBuilder().append("https://api.github.com/")
              .append("repos/")
              .append("Kawaxte/")
              .append("TwentyTenLauncher/")
              .append("releases").toString());
  ...
  public static Path getWorkingDir() {
    ...
    val workingDirLookup = Collections.unmodifiableMap(new HashMap<EPlatform, Path>() {
      {
        put(EPlatform.LINUX, Paths.get(userHome, ".twentyten"));
        put(EPlatform.MACOS, Paths.get(userHome, "Library", "Application Support", "twentyten"));
  ...
  public static String getManifestAttribute(String name) {
    ...
    try (val jarFile = new JarFile(new File(jarFileUrl.toURI()))) {
      ...
      logger.error("Failed to read manifest from {}",
          jarFileUrl,
          ioe);
    ...
    if (outdated == null) {
      val worker = new SwingWorker<Boolean, Void>() {
        ...
        protected Boolean doInBackground() {
          try {
            val request = Request.get(githubReleasesUrl.toURI())
                .addHeader("accept", "application/vnd.github+json")
                .addHeader("X-GitHub-Api-Version", "2022-11-28").execute();
      ...
      try {
        ...
        logger.error("Exception while checking for updates",
            ee);
        outdated = false;
      } catch (InterruptedException ie) {
        logger.error("Interrupted while checking for updates",
            ie);
        outdated = false;
      } finally {
        worker.cancel(true);
  ...
  public static void openBrowser(URL url) {
    try {
      ...
      LauncherUtils.logger.error("Failed to open '{}' in browser", url, ioe);
    } catch (URISyntaxException urise) {
  ...
  public static void openDesktop(Path p) {
    try {
      ...
      LauncherUtils.logger.error("Failed to open '{}' in desktop", p, ioe);
    }
  }
}
