...
public class MinecraftLauncher extends JApplet implements AppletStub {
  ...
  {
    this.titleString = LauncherLanguage.getString("ml.string.title");
    this.failedString = LauncherLanguage.getString("ml.string.title.fatal");
  }

  public MinecraftLauncher() {
    this.parameters = new HashMap<>();
    this.bgImage = FileHelper.readImageFile(MinecraftLauncher.class,
        "icon/dirt.png");
  }
  ...
  private void replace(Applet applet) {
    ...
    if (this.applet != null) {
      panel.add(this.applet, BorderLayout.CENTER);
    }
    ...
    LauncherFrame.instance.getContentPane().removeAll();
    LauncherFrame.instance.getContentPane().add(panel);
    panel.revalidate();
  ...
  private void drawTitleString(String s, int pWidth, int pHeight,
      Graphics2D g2d) {
  ...
  private void drawStateString(String s, int pWidth, int pHeight,
      Graphics2D g2d) {
  ...
  private void drawSubtaskMessageString(String s, int pWidth, int pHeight,
      Graphics2D g2d) {
    g2d.setFont(new Font(Font.SANS_SERIF, Font.PLAIN, 12));
    g2d.setColor(Color.LIGHT_GRAY);
  ...
  private void drawPercentageRect(int pWidth, int pHeight, Graphics2D g2d) {
    ...
    int rectWidthPercentage =
        (this.update.getTotalPercentage() * rectWidth) / 100;
    int rectHeight = 5;
  ...
  public void start() {
    ...
    final Timer paintTimer = new Timer(10, new ActionListener() {
      @Override
      public void actionPerformed(ActionEvent evt) {
        Object src = evt.getSource();
        if (MinecraftLauncher.this.applet == null) {
  ...
  public void paint(Graphics g) {
    ...
    VolatileImage compatibleVolatileImage = gc.createCompatibleVolatileImage(
    ...
    try {
      ...
      this.drawSubtaskMessageString(subtaskMessage, panelWidth, panelHeight,
          g2d);
    ...
    g.drawImage(compatibleVolatileImage, 0, 0, panelWidth, panelHeight, 0, 0,
